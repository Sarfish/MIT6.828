...
mem_init_mp(void)
{
	...
	int32_t stk_top_va = KSTACKTOP;
	for(int cpu_i = 0; cpu_i < NCPU; cpu_i ++){
		int32_t stk_top_pa = PADDR(percpu_kstacks[cpu_i]);
		boot_map_region(kern_pgdir, 
			stk_top_va - KSTKSIZE,
			KSTKSIZE,
			stk_top_pa,
...
page_init(void)
{
	...
	const size_t npages_iophy = IOPHYSMEM/PGSIZE, npages_ext = EXTPHYSMEM/PGSIZE;
	for(i = npages_iophy; i < npages_ext; i++){
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}

	//从内核到boot_alloc分配结束的页已在使用中
	size_t endofuse = ((size_t)ROUNDUP(PADDR(boot_alloc(0)), PGSIZE))/PGSIZE;
	
	for(; i<endofuse; i++){
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
	//注意：这里对挂载的顺序进行调整，使得内核初始化时
	//使用的0~4M的页面中的空闲页优先被调用，使得内核可以访问页表
	//在该实验的测试函数中隐含地实现了这一点
	//因此，如果不在这里进行调整，删除测试函数后会引起异常，因为内核
	//在建立内存映射前，无法访问由分配器随机分配的物理页
	for(i = 1024; i<npages; i++){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
	for(i = endofuse; i< 1024 ; i++){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
		//cprintf("add page 0x%0x on free list\n", page2pa(page_free_list));
	}
}
...
page_alloc(int alloc_flags)
{
	...
	if(alloc_flags & ALLOC_ZERO){
		void* page_kva = (void*)page2kva(free_page);
...
{
	// Fill this function in
	for(uintptr_t i = va, j = pa, k=0; k < size; k+=PGSIZE){
		tlb_invalidate(pgdir, (void *)(i + k));
		pte_t * pte = pgdir_walk(pgdir, (void*)(i + k), 1);
		*pte = (pa + k) | PTE_P | perm;
	}
}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if( ROUNDUP(base+size, PGSIZE) >= MMIOLIM)
		panic("mmio map region overflow the limit of MMIO region.");
