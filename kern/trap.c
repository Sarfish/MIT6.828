...
trap_init_percpu(void)
{
	...
	int32_t cpu_stack_top = KSTACKTOP - (KSTKSIZE + KSTKGAP) * cpu_id;
	thiscpu->cpu_ts.ts_esp0 = cpu_stack_top;
	...
	ltr(GD_TSS0 + sizeof(struct Segdesc) * cpu_id);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if((tf->tf_cs & 3) == 0){
		panic("Page fault in kernel. va = 0x%08x\n", fault_va);
	}
